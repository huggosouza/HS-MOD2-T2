...
class Game:
    ...
    def print_text(self, urText, pos):
        font = pygame.font.Font(FONT_STYLE, 22)
        text = font.render(f"{urText}", True, "#000000")
        text_rect = text.get_rect()
    ...
    def show_menu(self):
        ...
        if self.death_count == 0:
            ...
            text_rect = text.get_rect()
            text_rect.center = (HALF_S_WIDTH, HALF_S_HEIGHT)
            self.screen.blit(text, text_rect.center)
        else:
            self.screen.blit(ICON, (HALF_S_WIDTH, HALF_S_HEIGHT))
            self.print_text("You died, try again!", [HALF_S_WIDTH, HALF_S_HEIGHT - 50])
